...
def merge_lines(lines):
	for line_a in lines:
		...
		gradient_a = (ay2 - ay1) / (ax2 - ax1)
		for line_b in lines:
			bx1, by1, bx2, by2 = line_b
			gradient_b = (by2 - by1) / (bx2 - bx1)
			if line_a == line_b:
				continue
			has_same_gradient = abs(gradient_b - gradient_a) < 0.1
			if not has_same_gradient:
				continue
...
def preprocess_image(image):
	...
	filter_red_lines(image)
	# image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
	# apply_threshold(image, 140)
	return image
...
def get_red_lines(image):
	...
	lines = cv2.HoughLinesP(edges, 1, np.pi / 180, threshold=50, minLineLength=40, maxLineGap=3)
	cv2.imshow("Point Drawing", edges)
	cv2.waitKey(0)
	cv2.destroyAllWindows()
	# remove lines that don't have a gradient of 0.75 or -0.75
	processed_lines = []
	for line in lines:
		x1, y1, x2, y2 = line[0]
		x_length = x2 - x1
		...
		if abs(gradient) > 0.8 or abs(gradient) < 0.7:
			continue
