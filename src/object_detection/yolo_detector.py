...
class YoloDetector:
	...
	def predict(self, image_data) -> list[DetectorResult]:
		result = self._model.predict(source=image_data , verbose=False, conf=self.min_conf)[0]
		names = result.names
		boxes = result.boxes
		predict_results = []
		for conf, cls, xyxy, xyxyn, xywh, xywhn in zip(boxes.conf, boxes.cls, boxes.xyxy, boxes.xyxyn, boxes.xywh, boxes.xywhn):
			...
			cls = int(cls.item())
			name = names[cls]
			predict_results.append(DetectorResult(cls, conf, name, xyxy, xyxyn, xywh, xywhn))
