from src.cv.yolo_detector import YoloDetector
import numpy as np
import cv2
import math

building_detector = YoloDetector(
    "assets/or_models/building_detector_model.pt", 0.8)
...
class HomeBase:
    ...
    def load(self, image: np.ndarray):
        self.image = image
        self.buildings = building_detector.predict(image)
...
if __name__ == "__main__":
    hb = HomeBase()
    img = cv2.imread("src/cv_test/bs/test0.png")
    hb.load(img)
    rl = hb.red_lines()
    amount_on_line = 10
    for line in rl:
